...
fn libusb_err(rc: i32, hint: &str) -> PyErr {
    if rc == ffi::constants::LIBUSB_ERROR_PIPE {
        PyRuntimeError::new_err(format!("{hint}: device STALLed the control transfer — \
...
impl PyWaveformDev {
    fn ctrl_in(&self, b_request: u8, w_index: u16, length: u16) -> PyResult<Vec<u8>> {
        ...
        let n = unsafe {
            ffi::libusb_control_transfer(
                self.dev_handle, 0xC0, b_request, 0, w_index,
                buf.as_mut_ptr(), length, 1000,
            )
        };
        if n < 0 { return Err(libusb_err(n, &format!("control_transfer IN bRequest=0x{b_request:02x}"))); }
    ...
    fn ctrl_out(&self, b_request: u8, w_index: u16, data: &[u8]) -> PyResult<()> {
        let n = unsafe {
            ffi::libusb_control_transfer(
                self.dev_handle, 0x40, b_request, 0, w_index,
                data.as_ptr() as *mut u8, data.len() as u16, 1000,
            )
        };
        if n < 0 { return Err(libusb_err(n, &format!("control_transfer OUT bRequest=0x{b_request:02x}"))); }
        if n as usize != data.len() {
            return Err(PyRuntimeError::new_err(format!(
                "short control write: sent {} of {} bytes", n, data.len())));
        }
        Ok(())
    }
    ...
    fn channel_id(name: &str) -> PyResult<u16> {
        channel_id_from_name(name).ok_or_else(||
            PyValueError::new_err(format!("unknown channel {name:?} — use dac0..1, pwm0..7, dout0..15, din0..15, adc0..7")))
    }

    fn shape_code(name: &str) -> PyResult<u8> {
        match name.to_ascii_lowercase().as_str() {
            "dc" => Ok(1), "sine" => Ok(2), "square" => Ok(3),
            "triangle" => Ok(4), "sawtooth" => Ok(5),
...
impl PyWaveformDev {
    ...
    fn open() -> PyResult<Self> {
        unsafe {
            ...
            if r != 0 { return Err(PyRuntimeError::new_err(format!("libusb_init failed: {r}"))); }
    ...
    fn state(&self, py: Python<'_>, channel: &str) -> PyResult<PyObject> {
        ...
        d.set_item("shape_name", match s.shape {
    ...
    fn play_builtin(&self, channel: &str, shape: &str,
                    freq_hz: f64, amplitude: u16, offset: u16, duty: f32) -> PyResult<()> {
        let id = Self::channel_id(channel)?;
        let shape_code = Self::shape_code(shape)?;
        let spec = WaveBuiltinSpec {
            shape: shape_code, flags: 0,
    ...
    fn play_square(&self, channel: &str, freq_hz: f64, amplitude: u16, offset: u16, duty: f32) -> PyResult<()> {
    ...
    fn play_arbitrary(&self, channel: &str, samples: Vec<i16>,
                      sample_rate_hz: u32, loop_count: u16) -> PyResult<()> {
        let id = Self::channel_id(channel)?;
        if samples.is_empty() || samples.len() > 1024 {
            return Err(PyValueError::new_err("samples length must be 1..1024"));
        }
        let header = WaveArbHeader {
            n_samples: samples.len() as u16, loop_count, sample_rate_hz,
        };
    ...
    fn play_lut(&self, channel: &str, input_src: &str, input_arg: u16,
                entries: Vec<i16>, output_mask: u16) -> PyResult<()> {
        let id = Self::channel_id(channel)?;
        ...
        let header = WaveLutSpec {
            input_src: src, _reserved0: 0, input_arg,
            n_entries: entries.len() as u16, output_mask, _reserved1: 0,
        };
    ...
    fn play_threshold(&self, channel: &str, input_src: &str, input_arg: u16,
                      thr_high: u16, thr_low: u16,
                      val_high: u16, val_low: u16) -> PyResult<()> {
        let id = Self::channel_id(channel)?;
        let src = match input_src.to_ascii_lowercase().as_str() {
            "adc" => 1, "din" => 2,
            _ => return Err(PyValueError::new_err("input_src must be \"adc\" or \"din\"")),
        ...
        let spec = WaveThresholdSpec {
            input_src: src, _reserved0: 0, input_arg,
            thr_high, thr_low, val_high, val_low, _reserved1: 0,
        };
        let bytes = unsafe { std::slice::from_raw_parts(&spec as *const _ as *const u8, 16) };
    ...
    fn play_pulse_trig(&self, channel: &str, din_bit: u8, edge: &str,
                       active_level: u8, duration_us: u32, cooldown_us: u32) -> PyResult<()> {
        let id = Self::channel_id(channel)?;
        ...
        let spec = WavePulseSpec {
            input_din_bit: din_bit, edge: edge_v, active_level, _reserved0: 0,
            duration_us, cooldown_us,
        };
    ...
    fn play_pid(&self, channel: &str, input_arg: u16, setpoint: i32,
                kp: f32, ki: f32, kd: f32,
                sample_rate_hz: u32, out_min: u16, out_max: u16,
                integral_clamp: i32) -> PyResult<()> {
        let id = Self::channel_id(channel)?;
        let q16 = |f: f32| (f * 65536.0).round() as i32;
        let spec = WavePidSpec {
            input_src: 1, _reserved0: 0, input_arg,
            sample_rate_hz, setpoint,
            kp_q16_16: q16(kp), ki_q16_16: q16(ki), kd_q16_16: q16(kd),
            out_min, out_max, integral_clamp,
        };
