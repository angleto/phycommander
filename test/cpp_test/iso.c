...
setup_bm_device(void)
{
	...
	LIBUSB_CHECK(libusb_set_interface_alt_setting(handle, 0, 1));
...
write_data(libusb_device_handle *handle, size_t n)
{
	...
	unsigned char *p = create_dummy_data(n);
...
read_data(libusb_device_handle *handle, size_t n)
{
	...
	unsigned char *p = (uint8_t *) calloc(n, sizeof(uint8_t));
